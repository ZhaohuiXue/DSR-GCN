...
for (FLAG, curr_train_ratio, Scale) in [(1, 0.1,100)]:
    ...
    for smallscale in [40]:
        for sampless in [5]:
            ...
            Seed_List = [0, 1, 2, 3, 4,5,7,8,9,10 ] # 随机种子点
            ...
            for curr_seed in Seed_List:
                ...
                Qsmall, _, Asmall, _ = ls.simple_superpixel(scale=smallscale)
                ...
                channel = t
                Sptransnet = STnet.STnet(200,channel)
                ...
                net.to(device)
                summary(net, (90, 9, 9))
                GCNnet = DSRGCN.DSGCN(height, width, channel, class_count, Q, A,Qsmall,Asmall)
                GCNnet.to(device)
                ...
                SToptimizer = torch.optim.Adam(Sptransnet.parameters(), lr=5e-4, weight_decay=0.00005)
                optimizer = torch.optim.Adam(net.parameters(), lr=5e-4,weight_decay=0.00005)
                ...
                contrast_loader = torch.utils.data.DataLoader(dataset=contrast_data, batch_size=128, shuffle=True,
                                                              drop_last=False)
                labeled_trainloader = torch.utils.data.DataLoader(dataset=classidata, batch_size=32, shuffle=True,
                                                              drop_last=False)
                dataloadingend = time.time() -dataloadstart
                print('data loading time :%.4f'%dataloadingend)

                CNNtrainstart = time.time()
                contrastlearning(40, contrast_loader,labeled_trainloader,optim_contrast,optimizer, SToptimizer,Sptransnet ,net,lentrainsample,lencontrast,GCNnet,GCNDATA,optimizer1, train_samples_gt_onehot, train_label_mask)
                ...
                print('test time:%.2f'%testtime)
                print('testOA:',testOA)

                classification_map = torch.argmax(output, 1).reshape([height, width]).cpu()
                # entiremap = draw_result(classification_map,gtt, dataID=1)
                classification = output.cpu().detach().numpy()

                # plt.imsave('./result/image/'+str(sampless)+'sample'+ '_OA_' + str('%.2f'%(testOA * 10000)) + '.png',entiremap)
                torch.cuda.empty_cache()
            ...
            f = open('result/txt/' +str(train_samples_per_class) + 'sample.txt', 'a+')
            OA_results = '\n======================' \
