...
class SLIC(object):
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        # plt.figure()
        # plt.imshow(out)
        # # plt.imsave('./pcasuper' ,out/255)
        # plt.show()
        
        segments = np.reshape(segments, [-1])
        S = np.zeros([superpixel_count, d], dtype=np.float32)#超像素数 * 波段数
        Q = np.zeros([w * h, superpixel_count], dtype=np.float32)#像素数*超像素数
        x = np.reshape(img, [-1, d])#像素数*波段数
        
        for i in range(superpixel_count):
            idx = np.where(segments == i)[0]#找出每个超像素内像素的个数 N
            count = len(idx)
            pixels = x[idx]#将其在原图的位置的值找出    N*15
            superpixel = np.sum(pixels, 0) / count   #将每个波段上的所有像素值取平均 /N
            S[i] = superpixel
            Q[idx, i] = 1#将237个超像素分别以全图大小的分离开，并在超像素所在的位置赋值1
 #这一步后S为将237个像素块儿在15个波段上的平均值
        self.S = S
        self.Q = Q
    ...
    def get_A(self, sigma: float):
        ...
        for i in range(h - 2):
            for j in range(w - 2):#这里计算邻接关系的窗口为2*2
...
class LDA_SLIC(object):
    ...
    def LDA_Process(self,curr_labels):
        ...
        :return:
        '''
        height, width = curr_labels.shape
        ...
        # # #换成PCA试试
        # DATA, pca = applyPCA(self.data, numComponents=10)
        # DATA = np.reshape(DATA, [height * width, 10])
        # X_new = DATA
        # #这里对原图中有标签的部分进行了线性判别分析
        # m = X_new
        # t = np.reshape(m, [self.height, self.width, -1])
    ...
    def SLIC_Process(self, img, scale=25):
       ...
       #这里最终生成了segments就是超像素的个数也就是中心节点的个数
